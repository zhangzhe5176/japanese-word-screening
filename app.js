(() => {
  ...
  const STORAGE_KEY = "japaneseWordScreening:v1";
  ...
  app.innerHTML = '<section class="vv-panel vv-loading"><p class="vv-kicker">Japanese Word Screening</p><h1>正在读取 N5～N1 单词数据…</h1><p>只读取单词卡和 Topic 数据，不加载正文或媒体资源。</p></section>';

  window.loadVocabularyPrototypeData().then(boot).catch((error) => {
    app.innerHTML = `<section class="vv-panel vv-error"><p class="vv-kicker">Japanese Word Screening</p><h1>单词数据暂时无法加载</h1><p>${escapeHtml(error.message || "请通过本地预览服务打开网站。")}</p></section>`;
  });
  ...
  function makeModel(level, raw) {
    ...
    const rawWords = data.words.map((word) => ({ ...word, level }));
  ...
  function wordsForExportScope(scope = exportScope) {
    ...
    return model.words.filter((word) => topicIds.includes(Number(word.topicId)));
  }
  ...
  function render() {
    ...
    app.innerHTML = `
      <header class="vv-hero">
        <div><p class="vv-kicker">Japanese Word Screening · 独立站</p><h1>N5～N1 单词筛查与薄弱 Topic 提示</h1><p>每次只测试一个等级的一个 Topic；只保留单词卡、学习状态和导出功能。</p></div>
        <span class="vv-badge">独立网站</span>
      </header>
      ...
      <section class="vv-panel vv-data-management" aria-labelledby="vv-data-title"><div><p class="vv-kicker">Data management</p><h2 id="vv-data-title">数据管理</h2><p>重置只影响当前等级的单词筛查状态，不删除单词数据。</p></div><div class="vv-reset-actions"><button class="vv-reset-btn" type="button" data-reset-topic>重置当前 Topic</button><button class="vv-reset-btn" type="button" data-reset-level>重置当前 ${currentLevel.toUpperCase()} 等级</button></div></section>
  ...
  function exportWords(type, scope = exportScope) {
    ...
    const exportRows = rows.map((word) => {
      ...
      return [`Topic ${word.topicId} ${topic?.title || ""}`.trim(), word.sourceVocabNo, word.readingReview ? "" : word.kana, word.kanji, word.pos, word.meaning];
    });
    const blob = createXlsxBlob([["Topic", "词条编号", "假名", "日语汉字", "词性", "中文释义"], ...exportRows]);
